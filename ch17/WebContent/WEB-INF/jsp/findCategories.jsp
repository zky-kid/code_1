...
<c:set var="theListCategoriesWithProductsCount" value="${requestScope.listCategoriesWithProductsCount}" scope="page"/>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<body>
	<div style="width:1000px; margin:0px auto;">
		...
		<c:set var="theResultSize" value="${theListCategoriesWithProductsCount.size()}" scope="page"/>
		<c:if test="${theResultSize==0}"><div style="text-align:center">无类别信息</div></c:if>
		<c:if test="${theResultSize>0}">
	
			<table align="center">
				<tr>
					<th>序号</th><th>编号</th><th>类别名</th><th>商品种类数</th><th>修改</th><th>删除</th>
				</tr>
			<c:forEach items="${theListCategoriesWithProductsCount}" var="categoryWithProductsCount" varStatus="theStatus" >
				<c:set var="category" value="${categoryWithProductsCount.category}" scope="page"/>
				<c:set var="productsCount" value="${categoryWithProductsCount.productsCount}" scope="page"/>
				...
				<tr>
					...
					<td>${productsCount}</td>					
					<td><a href="#" onclick="formUpdateContentInitial(${theIndex+1}, ${theId})" >修改</a></td>
					<td>
					<c:if test="${productsCount==0}">
						<a href="${pageContext.request.contextPath}/deleteCategory?id=${theId}" onclick="return confirm('确定要删除？');">删除</a>
					</c:if>
					<c:if test="${productsCount>0}">
						不能删除
					</c:if>						
