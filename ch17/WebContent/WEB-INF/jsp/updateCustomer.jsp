...
function ajaxValidateCustomer(theFieldType) {
    ...
    $.ajax({
        type:"POST",
        url:"${pageContext.request.contextPath}/isOtherExistSame",//提交地址
        data:JSON.stringify({"fieldValue":fieldValue, "fieldType":theFieldType, "id": ${theCustomer.id} }),//提交数据  
...
function ajaxValidateCard() {
    ...
    $.ajax({
        type:"POST",
        url:"${pageContext.request.contextPath}/isOtherExistSameCard",//提交地址
        data:JSON.stringify({"fieldValue":fieldValue, "fieldType":cardType, "id":$("#theCardId").val()}),//提交数据  
        contentType : "application/json;charset=UTF-8",
        dataType:"json",//返回json格式        text 
        success:function(resultData){            
        	if (true == resultData) {//不能注册
            	...
            	$("#"+remindId).html("<font color='blue'>可以修改<font>");//如果没有被，将提示语清空。
            	existFlag=0;            	
            }
        }
    });
...
<body>
	<div style="width:900px; margin: 0px auto;">
		...
		<form action="${pageContext.request.contextPath}/doUpdateCustomer" method="post" onsubmit="return checkValid(this);">
			<input type="hidden" value="${theCustomer.id }" name="id" id="id">
			...
			<table align="center">
				...
				<tr>
					...
					<td>
						...
						<input type="hidden" value="${theCard.id }" name="card.id" id="theCardId">
						<fieldset>
							<legend>证件</legend>
							类型：<select name="card.cardType" id="cardType"><option value="0">身份证</option></select><br>
							号码：<input name="card.cardNo" id="cardNo" size="20" maxlength="30" value="${theCard.cardNo}" onkeyup="processInputStatus(this);" onblur="ajaxValidateCard()"><span id="showInputStatus"></span> <span id="cardNoRemind"></span><br>
							有效期：<input type="text" readonly='readonly' name='card.startDate' id="startDateId" value="${theCard.startDate}" class="Wdate" size="10" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', maxDate:'#F{$dp.$D(\'endDateId\')}'})" />-
                			<input type="text" readonly='readonly' name='card.endDate' id="endDateId" value="${theCard.endDate}" class="Wdate" size="10" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate:'#F{$dp.$D(\'startDateId\')}'})" /><br>
							发证机关：<input name="card.issuingAuthority" value="${theCard.issuingAuthority}">
						</fieldset>
					</td>
				</tr>				
				<tr>
					<th>Email</th><td><input type="text" name="email" id="email" value="${theCustomer.email }" onblur="ajaxValidateCustomer('2')"><span id="emailRemind"></span></td>
				</tr>
				<tr>
					<th>电话</th><td><input type="text" name="contactPhone" id="contactPhone" value="${theCustomer.contactPhone }" onblur="ajaxValidateCustomer('3')" size="30" maxlength="50"><span id="contactPhoneRemind"></span></td>
				</tr>
				<tr>
